...
func DecorateRecordType(info *ast.Record, t decorated.TypeAddAndReferenceMaker) (
	*dectype.RecordAtom, decorated.TypeError,
) {
	...
	for _, field := range info.Fields() {
		...
		log.Printf(
			"encountered %T %v", convertedFieldType,
			convertedFieldType.FetchPositionLength().ToCompleteReferenceString(),
		)
